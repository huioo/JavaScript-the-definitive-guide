...
/** 
 * 给动画方法传入回调函数，可以在动画结束时执行操作。不过，如果只是想顺序执行多段动画的话，回调方式是没有必要的。
 * jQuery动画默认是队列化的。
 * 如果一个元素已经在动画过程中，再调用一个动画方法时，新动画不会立刻执行，而会延迟到当前动画结束后才执行。
 * 例如，让一个元素在持久显示前，先闪烁一阵。
 */
 ...
 *      toggle()可以改变在上面调用它的元素的可视状态：如果隐藏、则调用show()；如果显示，则调用hide()。与show()、hide()一样，必须传入时长或选项对象
 *      给toggle()产生动画效果。给toggle()传入true和不带参数调用show()是一样的，传入false则和不带参数调用hide()一样的。
 *      注意，如果传入两个或多个函数参数给toggle()，它会注册为事件处理程序，这在19.4.1节讲过。
 * 
 * - slideDown()、slideUp()、slideToggle()
 *      slideUp()会隐藏jQuery对象中的元素，方式是将其高度动态变化到0，然后设置CSS的display属性为“none”。
 *      slideDown()执行反向操作，来使得隐藏的元素再次可见。
 *      slideToggle()使用向上滑动或向下滑动动画来切换元素的可见性。
 *      这三个方法都可接受可选的时长和回调函数（或选项对象参数）。
 */
// 调用该组方法中的每一个。jQuery动画默认情况是队列化的，因此这些动画会一个接一个执行。
