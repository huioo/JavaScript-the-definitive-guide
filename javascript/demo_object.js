 ...
 * 
 * 下面是使用Object.defineProperty()和Object.defineProperties()时的完整规则，任何违反规则的使用都会抛出类型错误异常：
 * - 如果对象是不可扩展的，则可以编辑已有的自有属性，但不能添加新属性
 * - 如果属性是不可配置的，则不能修改它的可配置性和可枚举型
 * - 如果“存取器属性”是不可配置的，则不能修改其getter和setter方法。
 * - 如果“存取器属性”是不可配置的，则不能将它转换为数据属性。
 * - 如果“数据属性”是不可配置的，则不能将它转换为存取器属性。
 * - 如果“数据属性”是不可配置的，则不能将它的可写性从false写为true，但可以从true修改为false
 * - 如果“数据属性”是不可配置且不可写的，则不能修改它的值。然而可配置但不可写属性的值是可以修改的（实际上是先将它标记为可写的，然后修改它的值，最后转换成不可写）
 */
